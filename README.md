...
<head>
    ...
    <style>
        ...
        body {
            ...
            min-height: 100vh;
            background: radial-gradient(circle, #fff0f3, #ffb3c1);
        ...
        .heart-fall {
            position: fixed; top: -10%; color: var(--primary-pink);
            font-size: 20px; user-select: none; z-index: 0;
        ...
        .container {
            ...
            z-index: 10; max-width: 90%; width: 500px;
        ...
        .btn-container {
            display: flex; justify-content: center; align-items: center;
            gap: 15px; margin-top: 25px; min-height: 120px;
        }
        ...
        #yesBtn { background-color: #2ecc71; color: white; font-size: 1.2rem; }
        #noBtn { background-color: #e74c3c; color: white; }
        ...
        .secret-area {
            ...
            background: #fdf2f4; padding: 10px; margin-top: 15px;
            border-radius: 10px; color: #ff4d6d; word-wrap: break-word;
        }

        input[type="password"] {
            padding: 12px; border-radius: 10px; border: 1px solid #ddd; width: 80%;
            margin-bottom: 10px; outline: none;
        }
    </style>
    <link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@500&family=Poppins:wght@300;600&display=swap" rel="stylesheet">
</head>
<body>

    <div id="passSection" class="container">
        <h2>Enter the Secret Key 🗝️</h2>
        <input type="password" id="passInput" placeholder="Password...">
        <p><small style="color: #888;">Hint: ilovemybabysomuch</small></p>
        <button onclick="checkPass()" style="background: var(--primary-pink); color: white;">Unlock My Heart</button>
    </div>

    <div id="proposalSection" class="container hidden">
    ...
    <div id="successSection" class="container hidden">
        <h1 style="color: var(--primary-pink); margin-bottom: 0;">THANK YOU BABYYY</h1>
        <p style="font-size: 0.9rem; color: #666;">You've made me the happiest person!</p>
        
        <div id="envelopeSection">
            <p>I have a message for you...</p>
            <button onclick="startTypewriter()" style="background: var(--gold); color: white; padding: 15px 30px; font-size: 1.1rem;">✉️ Open Virtual Envelope</button>
        </div>

        <div id="letterArea" class="hidden">
            <div id="typewriter" class="letter-content"></div>

            <button onclick="generateSecret()" style="background: #fff; border: 1px solid #ddd; color: #ff4d6d; margin-top: 20px; font-size: 12px; padding: 10px;">Click for a Secret</button>
            <div id="secretWall" class="secret-area hidden"></div>
        </div>
    </div>

    <script>
        // Your 1000-character letter content
        const myLetter = `My love,
...
Always yours 🤍`;
        ...
        // Shrinking/Growing Logic
        let noCount = 0;
        const noTexts = ["Are you sure?", "Really?", "Your baby will get sad ☹️"];
        ...
        function rejectProposal() {
            ...
            let currentNoSize = parseInt(window.getComputedStyle(noBtn).fontSize);
            let currentYesSize = parseInt(window.getComputedStyle(yesBtn).fontSize);
            
            noBtn.style.fontSize = Math.max(currentNoSize - 3, 2) + "px";
            noBtn.style.padding = "2px";
            yesBtn.style.fontSize = (currentYesSize + 20) + "px";
        }
        ...
        function startTypewriter() {
            ...
            let i = 0;
            const speed = 40; // Adjust speed in milliseconds
            const container = document.getElementById('typewriter');

            function type() {
                if (i < myLetter.length) {
                    container.textContent += myLetter.charAt(i);
                    i++;
        ...
        function generateSecret() {
            const wall = document.getElementById('secretWall');
